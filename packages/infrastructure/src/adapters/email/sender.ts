...
function transportOptions(smtp: SmtpConfig) {
  return {
    host: smtp.host,
    port: smtp.port,
    secure: smtp.secure,               // true = SSL from start (465); false = STARTTLS (587/25)
    requireTLS: !smtp.secure,          // on STARTTLS ports, fail if server doesn't offer STARTTLS
...
export async function testSmtpConnection(
  ...
  const transport = nodemailer.createTransport(transportOptions(smtp))
  try {
